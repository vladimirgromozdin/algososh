        ...
        <div className={styles.inputContainer}>
          ...
          <div className={styles.settingsSortingOrder}>
            <Button
              isLoader={isAdding}
              text="Добавить"
              disabled={inputValue === "" || !isFinished}
              onClick={() => handleAddToStack(delay)}
            ></Button>
            <Button
              isLoader={isRemoving}
              text="Удалить"
              disabled={visualStack.length === 0 || !isFinished}
        ...
        <div>
          {visualStack && (
            <ul className={styles.stack}>
              {visualStack.map((char, index) => (
                <li key={index}>
                  <Circle
                    state={
                      index === visualStack.length - 1 && !isFinished
